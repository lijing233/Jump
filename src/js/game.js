...
Game.prototype = {
  init: function (initBlockList) {
    ...
    this._initCreateCube(initBlockList, 'init'); // 初始化两个块
  ...
  // 游戏失败重新开始的初始化配置
  restart: function (initBlockList) {
    ...
    // this._createCube();
    // this._createCube();
    this._initCreateCube(initBlockList, 'restart'); // 初始化两个块
    this._createJumper();
    this._updateCamera();
  },
  // 游戏成功的执行函数, 外部传入
  ...
  _handleMouseup: function () {
    ...
    function landed() {
      ...
      if (self.falledStat.location === 1) {
        ...
        if (self.otherLandList.length > 0) {
          
          self.handleOtherLand(self.otherLandList[0])
          self.otherLandList.shift()
  ...
  handleOtherLand(cubeIndex) {
    ...
    if (self.flying) {
      console.log('正在飞行');
      self.otherLandList.push(cubeIndex)
      return
    }
  ...
  switchActiveUser(init) {
    var self = this;
    console.log("--------ssssssssssssssss----------", init);
  ...
  _initCreateCube: function (initBlockList, type) {
    const list = initBlockList || [
      Math.floor(Math.random() * (this.blockMapMax + 1)),
      Math.floor(Math.random() * (this.blockMapMax + 1)),
    ];
  ...
  _createCube: function (index) {
    ...
    if (this.cubes.length) {
      ...
      if (this.cubeStat.nextDir === "left") {
        ...
        mesh.position.x =
          this.cubes[this.cubes.length - 1].position.x - blockConfig.range - 6;
      } else {
        // mesh.position.z = this.cubes[this.cubes.length - 1].position.z-4*Math.random() - 6
        mesh.position.z =
          this.cubes[this.cubes.length - 1].position.z - blockConfig.range - 6;
      }
    }
